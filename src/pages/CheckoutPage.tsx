...
import { ArrowLeft, ShieldCheck, Truck, Wallet, Check } from 'lucide-react';
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      if (paymentMethod === 'COD') {
        const { data, error } = await supabase.functions.invoke('place-cod-order', { body: buildRequestBody() });
        if (error || data?.error) throw new Error(data?.error || error?.message || 'Could not place order');
        clearCart();
            ...
            <div className="space-y-3">
              <button
                ...
                className={cn(
                  ...
                  paymentMethod === 'Razorpay' ? 'border-ink-900 bg-ink-50' : 'border-ink-200 hover:border-ink-400'
                )}
              >
                <div className={cn('grid h-5 w-5 shrink-0 place-items-center rounded-full border-2', paymentMethod === 'Razorpay' ? 'border-ink-900' : 'border-ink-300')}>
                  {paymentMethod === 'Razorpay' && <div className="h-2.5 w-2.5 rounded-full bg-ink-900" />}
                </div>
              ...
              <button
                ...
                className={cn(
                  'flex w-full items-center gap-3 rounded-lg border p-4 text-left transition-all',
                  paymentMethod === 'COD' ? 'border-ink-900 bg-ink-50' : 'border-ink-200 hover:border-ink-400'
                )}
              >
                <div className={cn('grid h-5 w-5 shrink-0 place-items-center rounded-full border-2', paymentMethod === 'COD' ? 'border-ink-900' : 'border-ink-300')}>
                  {paymentMethod === 'COD' && <div className="h-2.5 w-2.5 rounded-full bg-ink-900" />}
                </div>
                <Wallet size={20} className="shrink-0 text-ink-600" />
                <div className="flex-1">
        ...
        <div>
          <div className="sticky top-24 rounded-xl border border-ink-100 p-6">
            ...
            <Button size="lg" className="mt-6 w-full" onClick={handlePlaceOrder} disabled={loading}>
              {loading ? <Spinner /> : <>{paymentMethod === 'COD' ? 'Place Order' : 'Pay Now'} <Check size={18} /></>}
            </Button>
